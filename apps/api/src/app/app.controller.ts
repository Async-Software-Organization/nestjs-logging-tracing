...
import { Tracing, TracingData, TracingInterceptor } from "@dollarsign/nestjs-jaeger-tracing";
...
export class AppController {
  ...
  getMovies(@Tracing() tracing: TracingData) {
    winstonLogger?.info(JSON.stringify(tracing));
  ...
  @UseInterceptors(TracingInterceptor)
  @Post("error")
  postError(@Tracing() tracing: TracingData) {
    winstonLogger?.error(JSON.stringify(tracing));
    return this.httpService.post(this.configService.get("apiDatabaseUrl") + "error", tracing);
  }
}
